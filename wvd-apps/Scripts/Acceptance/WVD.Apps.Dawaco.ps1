...
try {
    ...
    $RegisterFiles | ForEach-Object {

        Start-Process -FilePath "regsvr32.exe" -ArgumentList "/s", "`"$($_)`"" -NoNewWindow -Wait -ErrorAction Continue -Verbose
    }
...
#region Dawaco ArtDiver.ini
$ArtDiverIni = @"
[User]
PWN
[LastDiverBeheerdir]
\\pwnkaadawacofs.file.core.windows.net\dawaco\ArtDiv
[LastMonIdcDir]
\\pwnkaadawacofs.file.core.windows.net\dawaco\Programs
[LastMergeDir]
\\pwnkaadawacofs.file.core.windows.net\dawaco\ArtDiv
[LastExportDir]
\\pwnkaadawacofs.file.core.windows.net\dawaco\ArtDiv
"@
$ArtDiverIni | Set-Content -Path "$($InstallPath)\ArtDiver.ini" -Force -Verbose
#endregion
